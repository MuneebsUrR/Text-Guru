...
if(normalText.length>0){
    ...
    try {
        ...
        props.alert('success','You text paraphrased successfully :)')
    } catch (error) {
        console.error(error);
    }
}else{
        ...
        <div className='container my-5 paraphraser-section'>
            ...
            <div className="col-md-6 box-text">
               <div className="header text-secondary text-center">
                ...
                <textarea className='form-control' name="simpletext" value={normalText} onChange={(e)=>setnormalText(e.target.value)} cols="28" rows="10" style={{overflow:'auto'}}></textarea>
               
                <button  className={`btn ${normalText.length>=10000?'btn-outline-danger':'btn-outline-dark'}  my-3`} id='paraphrase-button' onClick={paraphrase} disabled={normalText.length>=10000?true:false } >Paraphrase Text</button>
            ...
            <div className="col-md-6 box-paraphrase">
                <div className="header text-secondary text-center ">
                   <span className="paraphrase-text">Paraphrase Text</span> 
                    <hr />
                    <textarea id='paraphrasetextarea' className='form-control' name="simpletext" value={paraphraseText} onChange={(e)=>setparaphraseText(e.target.value)} cols="28" rows="10" style={{overflow:'auto'}}></textarea>
                </div>
